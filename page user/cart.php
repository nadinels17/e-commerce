...
$q1 = mysqli_query($conn, "SELECT * FROM shop WHERE id_produk='$id'");
// $q1 = mysqli_query($conn, "SELECT * FROM shop INNER JOIN keranjang ON keranjang.id_produk=shop.id_produk");
// $q2 = mysqli_query($conn, "SELECT * FROM shop WHERE id_produk='$id'");
// $data1 = mysqli_fetch_array($q1);

$query = mysqli_query($conn, "INSERT INTO keranjang VALUES('', '$id', '17')");
...
<div class="container">
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <td><?= $no++;?></td>
                <td><img src="../pageadmin/dsbadmin/image/<?= $data['gambar']?>" alt="" width="300px"></td>
                <td><?= $data['nm_produk'];?></td>
                <td><?= $data['harga'];?></td>
                <td><a href="checkout.php?id_produk=<?=$data['id_produk'];?>" class="btn btn-danger me-2   ">Checkout</a></td>
            </tr>
            <?php endforeach;?>
        </tbody>
    </table>
