...
if (isset($_POST['search'])) {
    $search = $_POST['cari'];
    $query = mysqli_query($conn, "SELECT * FROM suppliers WHERE nama_sup LIKE '%$search%'");
...
<div class="row">
  ...
  <div class="col"></div>
    ...
    $data = mysqli_query($conn, "SELECT * FROM suppliers");
    $jumlahsupp= mysqli_num_rows($data);
  
  ?>

  <p style="font-size: 20px; color: grey;">Jumlah Suppliers : <?=$jumlahsupp;?></p>
...
<table class="table">
  ...
  <?php foreach ($query as $data2) :?>
    <tr class="table-danger">
      ...
      <td><?= $data2['jumlah'];?></td>
      <td><?= $data2['jenisbahan'];?></td>
      <td>
        <a class="btn btn-danger" href="delete.php?id_supp=<?=$data2['id_supp'];?>">DELETE</a><br><br>
        <a class="btn btn-info" href="edit_data.php?id_supp=<?=$data2['id_supp'];?>">EDIT</a>
      </td>
